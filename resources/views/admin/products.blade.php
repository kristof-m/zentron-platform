...
<head>
    <meta charset="UTF-8"/>
    @vite(['resources/css/style.css', 'resources/css/admin.css', 'resources/css/auth.css', 'resources/js/filter.ts'])
    <link rel="icon" type="image/svg+xml" href="/vite.svg"/>
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <meta name="description" content="zentron admin manage products"/>
...
<div style="width: 100%; padding-inline: 3rem; margin-bottom: 1rem; text-align: left;">
    <a class="apply-button" href="{{ route('product.new') }}" style="display: inline-flex; align-items: center; justify-content: center; text-decoration: none;">Add Product</a>
</div>
...
<section class="filter-bar" aria-label="List controls">
    ...
    @if($products->onFirstPage())
        <p class="icon-button" aria-disabled="true">
            <img src="{{ Vite::asset('resources/icons/arrow_left.svg') }}" alt="Previous page unavailable!" />
        </p>
    @else
...
<main class="products">
    @foreach($products as $product)
        <x-product-card :product="$product" :is-admin="true"/>
    @endforeach
</main>
